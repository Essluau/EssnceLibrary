...
function Library:CreateWindow(config)
	...
	local function setPopupActive(targetGui, isActive)
		local current = targetGui
		while current and current ~= ScreenGui do
			if current:IsA("GuiObject") then
				if isActive then
					if not current:GetAttribute("OrigZIndex") then
						current:SetAttribute("OrigZIndex", current.ZIndex)
					end
					local openCount = (current:GetAttribute("OpenPopups") or 0) + 1
					current:SetAttribute("OpenPopups", openCount)
					current.ZIndex = 200
				else
					local openCount = math.max(0, (current:GetAttribute("OpenPopups") or 1) - 1)
					current:SetAttribute("OpenPopups", openCount)
					if openCount == 0 then
						local orig = current:GetAttribute("OrigZIndex")
						if orig ~= nil then
							current.ZIndex = orig
							current:SetAttribute("OrigZIndex", nil)
						end
					end
				end
			end
	...
	local function shouldOpenUpwards(guiObj, requiredHeight)
		local limitBottom = 1e5
		local limitTop = 0
		local current = guiObj.Parent
		while current do
			if current:IsA("ScrollingFrame") or (current:IsA("GuiObject") and current.ClipsDescendants) or current == MainFrame then
				local bottom = current.AbsolutePosition.Y + current.AbsoluteSize.Y
				if bottom < limitBottom then
					limitBottom = bottom
				end
				local top = current.AbsolutePosition.Y
				if top > limitTop then
					limitTop = top
				end
			end
			if current == MainFrame then break end
		...
		local spaceBelow = limitBottom - (guiObj.AbsolutePosition.Y + guiObj.AbsoluteSize.Y)
		local spaceAbove = guiObj.AbsolutePosition.Y - limitTop
		return (spaceBelow < (requiredHeight + 6)) and (spaceAbove > spaceBelow)
	end
	...
	local function createSingleColorPicker(parentRow, offsetX, defaultColor, onColorChanged)
		...
		local function openPicker()
			...
			local pickerH = 144
			local openUp = shouldOpenUpwards(cpSlot, pickerH)
			if openUp then
				pickerFrame.AnchorPoint = Vector2.new(0, 1)
				pickerFrame.Position = UDim2.new(1, -162, 0, -4)
			...
			setPopupActive(cpSlot, true)
			pickerFrame.Visible = true
			TweenService:Create(pickerFrame, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0, 162, 0, pickerH)}):Play()
		end
		...
		if rightType == "colorbox" then
			...
			ConfigFeatureEntries[flag .. "_Color1"] = {
				Get = function()
					...
					return { R = col.R, G = col.B, B = col.B }
				end,
				Set = function(val)
	...
	local function createDropdown(parent, title, defaultVal, options, zIndexVal, onSelected, flag)
		...
		local function openDropdown()
			...
			local openUp = shouldOpenUpwards(box, fullHeight)
			if openUp then
				listFrame.AnchorPoint = Vector2.new(0, 1)
				listFrame.Position = UDim2.new(0, 0, 0, -3)
			...
			for i = 1, math.min(#configs, 3) do
				...
				elseif cfg.type == "dropdown" then
					...
					local function openD()
						...
						local pFullH = #opts * 18 + 4
						local openUp = shouldOpenUpwards(dBox, pFullH)
						if openUp then
